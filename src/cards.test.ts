...
const isRecordEntry = (
  ...
  isRecord(value[SECOND]);

const isRecordEntries = (
  value: unknown,
): value is readonly [string, Record<string, unknown>][] =>
...
const BANNED_TEXT: Record<Banned, string> = {
  ...
  [Banned.KamigawaCost]:
    'It would be too expensive to open so many Kamigawa booster packs.',
  [Banned.RemoveSleeves]:
    'Do not encourage players to remove uncube cards from their sleeves.',
  [Banned.DamageSleeves]:
    "Do not encourage players to damage the uncube's sleeves.",
  [Banned.Host]:
    'The uncube does not contain enough host cards or cards with augment to support this card.',
  [Banned.RandomCreature]:
    'Selecting random creature cards from a library is burdensome.',
  [Banned.Toys]: 'Uncube should not require toys.',
  [Banned.VisualAccessibility]:
    'This card is not accessible to visually-impaired players.',
  [Banned.Conjure]: 'Tracking conjured cards is burdensome in paper.',
  [Banned.Contraption]:
    'The uncube does not contain enough Contraptions to support this card.',
  [Banned.Outside]: 'Playing should not require a person outside the game.',
  [Banned.Consume]:
    'Do not encourage players to consume food or beverage near the uncube.',
  ...
  [Banned.Racism]:
    'This card may be banned due to its racist depiction, text, or combination thereof.',
};

const PROXY_TEXT: Record<Proxy, string> = {
  [Proxy.Racism]:
    'This card may be proxied due to its racist depiction, text, or combination thereof.',
};
...
describe('cards', (): void => {
  ...
  it('should describe every set', (): void => {
    ...
    for (const [cardName, { sets: cardSets }] of cardEntries) {
      assert(
        Array.isArray(cardSets),
        `Expected card "${cardName}" to have sets.
